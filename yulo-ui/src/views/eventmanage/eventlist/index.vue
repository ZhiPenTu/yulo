    ...
    <el-card class="toolbar" shadow="never">
      <el-form :inline="true" :model="query" label-width="80px">
        <el-form-item label="事件类型">
          <el-select v-model="query.eventType" placeholder="选择事件类型" clearable>
            <el-option label="拥堵" value="congestion" />
            <el-option label="超速" value="overspeed" />
            <el-option label="逆行" value="retrograde" />
          </el-select>
        </el-form-item>
        ...
        <el-form-item>
          <el-upload
            action="/eventManage/event/upload"
            :http-request="customUpload"
            :show-file-list="false"
            accept="image/*"
          >
            <el-button type="success">上传事件图片</el-button>
          </el-upload>
        </el-form-item>
      </el-form>
    </el-card>
      ...
      <el-table :data="list" style="width: 100%" v-loading="loading">
        ...
        <el-table-column label="路方目标类型" width="120">
              <template #default="{ row }">
                  <el-tag v-if="row.eventDetail.length>0&&row.eventDetail[0].classification_category =='roadCrew'"   type="primary">路方车辆</el-tag>
                  <el-tag v-else-if="row.eventDetail.length>0&&row.eventDetail[0].classification_category =='socialVehicle'" type="primary">社会车辆</el-tag>
            </template>
        </el-table-column>
        <el-table-column prop="laneId" label="车道号" width="120">
             <template #default="{ row }">
                  <el-tag v-for="item in row.eventDetail" :key="item.track_id" type="primary">{{ item.lane_name }}</el-tag>
            </template>
        </el-table-column>
          <el-table-column label="事件图片" width="160">
          <template #default="{ row }">
            <el-image
              v-if="row.eventPicurl"
              :src="row.eventPicurl"
              fit="cover"
              style="width: 140px; height: 80px; cursor: pointer;"
              :preview-src-list="buildPreviewList(row)"
        ...
        <el-table-column prop="endTime" label="结束时间" width="180">
            <template #default="{ row }">
              ...
              <el-icon v-else style="font-size: 18px;" class="is-loading"><Loading /></el-icon>
            </template>
...
<style scoped>
.event-list-page { padding: 12px; }
.toolbar { margin-bottom: 12px; }
.pagination { margin-top: 12px; display: flex; justify-content: flex-end; }
.image-slot { width: 140px; height: 80px; display: flex; align-items: center; justify-content: center; background: #f5f7fa; color: #909399; }
.video-trigger { cursor: pointer; display: inline-flex; align-items: center; }
.video-trigger.disabled { cursor: not-allowed; opacity: 0.4; }
